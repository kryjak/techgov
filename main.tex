...
\begin{abstract}
	In preparation for potential international agreements on artificial intelligence, the development of verification infrastructure is vital. We propose a method for fingerprinting AI cluster activity based on tapping network traffic in and out of datacenters. These fingerprints enable an auditor (the “verifier”) to retroactively challenge the original data to be sent to a privacy-preserving verification facility. Our main goal is to make it infeasible to covertly exfiltrate results of undisclosed workloads in the cluster. To this end, we propose using a retrofittable “border patrol” device configured to be the only communication channel between the datacenter and the outside world. Our contribution is the architecture and simulation of such a device, which also handles erasure of those covert channels which post-hoc verification on fingerprints cannot address: Analog and timing side-channels, as well as steganography in network protocol headers. The architecture eliminates the need for any mutually trusted processors, leveraging the physics of simple, passive optical fiber splitters and coin-flip protocols for random number generation where needed. We expect development costs of a demonstration device to be well below \$2M\textcolor{red}{REMEMBER THIS NEEDS JUSTIFICATION}. We make available our code at \href{https://github.com/derpyplops/warden}{https://github.com/derpyplops/warden}, as well as a visualisation of the device function at \href{https://www.felix-krueckel.com/warden.html}{https://www.felix-krueckel.com/warden.html}.
\end{abstract}
...
\begin{enumerate}
	\item A detailed description of the problem statement and the design requirement for a verification mechanism meant to address Subgoal 2A of~\citet{rand_verification} (\cref{sec:problem}).
	\item An actionable design specification of our proposed Border Patrol device that addresses these requirements. This includes a detailed description of how the device processes captured traffic in order to eliminate steganographic degrees of freedom in egress data (\cref{methods}).
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item The Verifier must not have the ability to extract Prover's proprietary information out of the device.
	\end{itemize}
	...
	\item The device should introduce as little additional latency as possible in order to minimise the slowdown of usual data centre operations. It should be able to process millions of communication packets streaming in/out every second.
\end{enumerate}
...
\begin{itemize}
	\item
	We focus on traffic serving LLM API endpoints to external customers. We also assume most API traffic is egress, not ingress. This is because customer traffic served via the `streaming API' format, commonly used for chatbot applications, is optimised for low latency in delivering responses to customers at the cost of high data inefficency (see e.g.,~\citet{openai_chat_completions_streaming}). Every single output token is wrapped in a JSON packaging adding $\sim$200 bytes of overhead\footnote{Strictly speaking, streaming responses may contain batches of a few tokens, although in practice the number is exactly one. We take this simplifying assumption because we are also interested in a worst-case feasilibity assessment of the Border Patrol solution. With only 1 token per streaming response, the amount of data produced represents an upper bound for the storage requirements of our method.}.
	\item
	Steganography via \textit{input} payload data (i.e., the plaintext requests to the LLM) is unconstrained\footnote{The warden device could in principle rewrite prompts and input media files while preserving semantics. However, this carries complications related to a potential degradation of the LLM's performance. We leave this open for future research.}. \textcolor{gray}{However, the output payload's strict causal dependence on the \textbf{declared} inputs is verified post-hoc, based on the evidence captured by the border patrol device. Only payload data is hashed, as protocol header nonces would otherwise need to be stored long-term} \textcolor{red}{I DON'T THINK THIS SHOULD BE HERE.}
	\item
	The Prover needs to store all information that forms a part of the hash for as long as is required by the Verifier. In practice, this includes both the plaintext response data and the associated JSON metadata.
\end{itemize}
...
\end{figure}
\begin{itemize}
	\item We consider only text inputs and outputs. Audio, image and video generation are out of scope. These could potentially increase the storage requirements of our method, as well as introduce novel steganographic degrees of freedom that ought to be accounted for.
\end{itemize}
